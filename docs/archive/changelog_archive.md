...
> Archived from `docs/changelog.md` on 2026-06-09 to keep the active changelog compact.
> Use targeted search for older entries instead of loading this whole file by default.
...
- `test_instrument_accounting_identity_holds` - identity holds with point_value -???1 and non-zero commission.
...
| 7 | Configure instrument profile | WARNING??| Default `config/instruments.json` template only |
...
| 11 | Show equity curve | WARNING??| Structured `equity_curve` + `drawdown_curve` produced; not yet charted |
